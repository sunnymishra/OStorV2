...
public class Metadata {
    ...
    public static Metadata deserializeMetadata(JsonNode entryNode) throws JsonParseException, IOException {
		String f_name = entryNode.path("name").asText();
		if (f_name == null) {
			throw new JsonParseException("Required field \"name\" missing.", null);
		}
		String f_pathLower = entryNode.path("path_lower").asText();
		String f_pathDisplay = entryNode.path("path_display").asText();
		String f_parentSharedFolderId = GenericDeserializerUtil.getNullableText(entryNode, "parent_shared_folder_id");
		// f_parentSharedFolderId =
		// StoneSerializers.nullable(StoneSerializers.string()).deserialize(p);
		Metadata value = new Metadata(f_name, f_pathLower, f_pathDisplay, f_parentSharedFolderId);
		return value;
	}
}
