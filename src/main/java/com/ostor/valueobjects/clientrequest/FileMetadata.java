...
import com.ostor.valueobjects.properties.PropertyGroup;
import com.ostor.valueobjects.serializer.FileMetadataDeserializer;
import com.ostor.valueobjects.serializer.GenericDeserializerUtil;
...
public class FileMetadata extends Metadata {
    ...
    public FileMetadata(String name, String id, Date clientModified, Date serverModified, String rev, long size, String pathLower, String pathDisplay, String parentSharedFolderId, MediaInfo mediaInfo, FileSharingInfo sharingInfo, List<PropertyGroup> propertyGroups, Boolean hasExplicitSharedMembers) {
        ...
        this.clientModified = com.ostor.util.LangUtil.truncateMillis(clientModified);
        if (serverModified == null) {
            throw new IllegalArgumentException("Required value for 'serverModified' is null");
        }
        this.serverModified = com.ostor.util.LangUtil.truncateMillis(serverModified);
        if (rev == null) {
    ...
    public static FileMetadata deserializeFileMetadata(JsonNode entryNode) throws IOException {
		String f_name = entryNode.path("name").asText();
		String f_id = entryNode.path("id").asText();
		Date f_clientModified = GenericDeserializerUtil.deserializeTimestamp(entryNode.path("client_modified").asText());
		Date f_serverModified = GenericDeserializerUtil.deserializeTimestamp(entryNode.path("server_modified").asText());
		String f_rev = entryNode.path("rev").asText();
		Long f_size = entryNode.path("size").asLong();
		String f_pathLower = GenericDeserializerUtil.getNullableText(entryNode, "path_lower");
		String f_pathDisplay = GenericDeserializerUtil.getNullableText(entryNode, "path_display");
		String f_parentSharedFolderId = GenericDeserializerUtil.getNullableText(entryNode, "parent_shared_folder_id");
		 /*MediaInfo f_mediaInfo = entryNode.path("media_info").asText();
		 FileSharingInfo f_sharingInfo = entryNode.path("sharing_info").asText();
		 List<PropertyGroup> f_propertyGroups = entryNode.path("property_groups").asText();
		 Boolean f_hasExplicitSharedMembers = entryNode.path("has_explicit_shared_members").asText();*/
		...
		// StoneSerializers.nullable(StoneSerializers.string()).deserialize(p);

		if (f_name == null) {
			throw new JsonParseException("Required field \"name\" missing.", null);
		}
		if (f_id == null) {
			throw new JsonParseException("Required field \"id\" missing.", null);
		}
		if (f_clientModified == null) {
			throw new JsonParseException("Required field \"client_modified\" missing.", null);
		}
		if (f_serverModified == null) {
			throw new JsonParseException("Required field \"server_modified\" missing.", null);
		}
		if (f_rev == null) {
			throw new JsonParseException("Required field \"rev\" missing.", null);
		}
		if (f_size == null) {
			throw new JsonParseException("Required field \"size\" missing.", null);
		}
		FileMetadata value = new FileMetadata(f_name, f_id, f_clientModified, f_serverModified, f_rev, f_size,
				f_pathLower, f_pathDisplay, f_parentSharedFolderId, f_mediaInfo, f_sharingInfo, f_propertyGroups,
				f_hasExplicitSharedMembers);
		return value;
	}
